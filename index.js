...
function MenuElemaniOlustur(x, y, z) {
  const w = { isim: x, fiyat: y, kategori: z };
  return w;
}
...
function DegerlendirmeEkle(x, i, p, g) {
  const depo = { isim: i, puan: p, geribildirim: g };
  x.push(depo);
  return x;
}
...
function AnahtardanDegerlendirmeAl(x, i) {
  let a =
    x[i].isim +
    " isimli kişi " +
    x[i].puan +
...
function SonDegerlendirmeyiAl(x) {
  let a =
    x[x.length - 1].isim +
    " isimli kişi " +
    x[x.length - 1].puan +
    " puan verdi ve şunları yazdı: " +
    x[x.length - 1].geribildirim;
  return a;
}
